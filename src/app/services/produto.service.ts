...
export class ProdutoService {
  ...
  getProduto(theProdutoId: number): Observable<Produto> {
    const produtoURL = `${environment.baseUrl}/api/produtos/${theProdutoId}`
  ...
  getProdutoListPaginate(thePage: number,thePageSize: number, categoriaId: number): Observable<GetResponseProdutos> {
    const searchURL = `${environment.baseUrl}/api/produtos/search/findByCategoriaId?id=${categoriaId}`+`&page=${thePage}&size=${thePageSize}`

    return this.httpClient.get<GetResponseProdutos>(searchURL);
  }

  getProdutoList(categoriaId: number): Observable<Produto[]> {
    const searchURL = `${environment.baseUrl}/api/produtos/search/findByCategoriaId?id=${categoriaId}`

    return this.getProdutos(searchURL);
  }

  searchProdutos(theKeyword: string): Observable<Produto[]> {
    const searchURL = `${environment.baseUrl}/api/produtos/search/findByNomeContaining?nome=${theKeyword}`

    return this.getProdutos(searchURL);
  }

  searchProdutosPaginate(thePage: number,thePageSize: number, theKeyword: string): Observable<GetResponseProdutos> {
    const searchURL = `${environment.baseUrl}/api/produtos/search/findByNomeContaining?nome=${theKeyword}`+`&page=${thePage}&size=${thePageSize}`
  ...
  getProdutoCategorias(): Observable<ProdutoCategoria[]> {
    const searchURL = `${environment.baseUrl}/api/produto-categoria`
